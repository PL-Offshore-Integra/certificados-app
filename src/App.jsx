...
const api = {
  async getCertificados() {
    const { data, error } = await supabase.from("certificados").select("*").eq("activo", true).order("buque").order("tipo").order("seccion").order("descripcion");
  ...
  // ── SUBVENCIMIENTOS ────────────────────────────────────────────────────────
  async getSubvencimientos(certId) {
    const { data, error } = await supabase.from("subvencimientos").select("*").eq("certificado_id", certId).order("fecha_desde");
    if (error) throw error;
    return data || [];
  },
...
:root{
  ...
  --green-light:#D1FAE5;--green-dark:#065F46;--green-border:#A7F3D0;
...
.sv{color:var(--blue)}.sr{color:var(--danger)}.sw{color:var(--warn)}
...
/* ── SUBVENCIMIENTOS ── */
.sv-block{background:#F0FDF4;border-left:3px solid #4ADE80;border-radius:0 var(--r) var(--r) 0;padding:0}
.sv-header{display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#166534;cursor:pointer;user-select:none}
...
@media print{
  ...
  .print-page table{font-size:10px}
  .sidebar,.topbar,.filter-row,.btn-print{display:none!important}
...
.print-modal{background:#fff;width:100%;max-width:780px;margin:auto;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,.25);overflow:hidden}
.print-modal-header{background:var(--navy);color:#fff;padding:14px 20px;display:flex;align-items:center;justify-content:space-between}
.print-body{padding:24px 28px}
.print-vessel-title{font-size:22px;font-weight:700;color:var(--navy);border-bottom:2px solid var(--navy);padding-bottom:8px;margin-bottom:4px}
.print-vessel-sub{font-size:11px;color:var(--muted);margin-bottom:14px;display:flex;justify-content:space-between}
.print-sec-header{background:var(--navy);color:#fff;font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:4px 10px;margin:14px 0 0;display:flex;align-items:center;gap:8px}
.print-table{width:100%;border-collapse:collapse;font-size:10px;margin-bottom:0}
.print-table th{font-size:8px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:var(--muted);padding:5px 8px;text-align:left;border-bottom:1.5px solid var(--border);background:var(--surface2);white-space:nowrap}
.print-table td{padding:5px 8px;border-bottom:0.5px solid var(--border);vertical-align:top;font-size:10px}
.print-table tr:last-child td{border-bottom:none}
.print-sv-row{background:#F9FFFE}
.print-sv-row td{padding:3px 8px 3px 24px;font-size:9px;color:var(--muted);border-bottom:1px dashed #BBF7D0}
.print-sv-arrow{color:#4ADE80;margin-right:4px;font-weight:700}
.print-footer{margin-top:16px;border-top:1px solid var(--border);padding-top:8px;display:flex;justify-content:space-between;font-size:9px;color:var(--muted);font-family:var(--mono)}
.print-chip{display:inline-flex;font-family:var(--mono);font-size:8px;font-weight:700;padding:2px 6px;border-radius:3px}
...
@media(min-width:701px){.mobile-cards{display:none}}
.mobile-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--r2);padding:12px 14px;margin-bottom:8px;cursor:pointer;transition:all .15s}
.mobile-card:hover{box-shadow:0 2px 8px rgba(33,51,99,.1)}
...
@media(max-width:700px){.hamburger{display:flex;align-items:center}}
.overlay-menu{display:none}
@media(max-width:700px){.overlay-menu.open{display:block;position:fixed;inset:0;background:rgba(33,51,99,.4);z-index:140}}
`;
...
function PrintChip({ fechaStr }) {
  const dias = diasHasta(fechaStr);
  if (dias === null) return <span className="print-chip" style={{ background: "#F3F4F6", color: "#6B7280" }}>Sin fecha</span>;
  const [bg, col] = dias < 0 ? ["#FEE2E2", "#991B1B"] : dias <= 30 ? ["#FFEDD5", "#9A3412"] : dias <= 90 ? ["#FEF3C7", "#92400E"] : ["#D1FAE5", "#065F46"];
  const label = dias < 0 ? `Venc. ${Math.abs(dias)}d` : `${dias}d`;
  return <span className="print-chip" style={{ background: bg, color: col }}>{label}</span>;
}
      ...
      <div className="login-card">
        ...
        <form onSubmit={handleLogin}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading} style={{ width: "100%", justifyContent: "center", marginTop: 6, padding: "10px 14px", fontSize: 13 }}>
...
function ModalSubvencimiento({ certId, sv, onClose, onSave, notify }) {
  ...
  const [form, setForm] = useState({
    ...
    fecha_desde: sv?.fecha_desde || "",
    fecha_hasta: sv?.fecha_hasta || "",
    observaciones: sv?.observaciones || "",
    documento_url: sv?.documento_url || "",
    documento_nombre: sv?.documento_nombre || "",
  });
  ...
  const handleUpload = async (file) => {
    if (!file) return;
    const tempId = sv?.id || `sv_temp_${Date.now()}`;
    setUploading(true);
    try {
      const { url, nombre } = await api.subirDocSubvencimiento(file, tempId);
      set("documento_url", url); set("documento_nombre", nombre);
      notify("Documento adjuntado", "success");
    } catch (e) { notify("Error al subir: " + e.message, "error"); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let saved;
      if (sv?.id) saved = await api.updateSubvencimiento(sv.id, payload);
      else saved = await api.insertSubvencimiento(payload);
      notify(esNuevo ? "Verificación creada" : "Verificación actualizada", "success");
      onSave(saved);
  ...
  return (
    <div className="overlay" style={{ zIndex: 200 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 520 }}>
        <div className="mhdr">
          <div className="mtitle">{esNuevo ? "Nueva verificación / subvencimiento" : "Editar verificación"}</div>
          <button className="mclose" onClick={onClose}>✕</button>
        </div>
        <div className="mbody">
          <FG label="Descripción *" full>
            <input value={form.descripcion} onChange={e => set("descripcion", e.target.value)} placeholder="Ej: 1ra Verificación anual, Inspección intermedia..." />
          </FG>
          <div className="form-grid mt12">
            <FG label="Fecha desde (inicio ventana)">
              <input type="date" value={form.fecha_desde} onChange={e => set("fecha_desde", e.target.value)} />
            </FG>
            <FG label="Fecha hasta (cierre ventana / vencimiento)">
              <input type="date" value={form.fecha_hasta} onChange={e => set("fecha_hasta", e.target.value)} />
            </FG>
          </div>
          <FG label="Observaciones">
            <textarea value={form.observaciones} onChange={e => set("observaciones", e.target.value)} placeholder="Notas, condiciones, resultado de la verificación..." />
          </FG>
          <div style={{ marginTop: 14 }}>
            ...
            {form.documento_url ? (
              <div className="doc-adjunto">
                <span>📎</span>
                <a href={form.documento_url} target="_blank" rel="noreferrer" style={{ color: "var(--blue)", flex: 1 }}>{form.documento_nombre || "Ver documento"}</a>
                <button onClick={() => { set("documento_url", ""); set("documento_nombre", ""); }} style={{ background: "none", border: "none", color: "var(--muted2)", cursor: "pointer" }}>✕</button>
              </div>
            ) : (
              <button className="btn btn-ghost btn-sm" onClick={() => fileRef.current.click()} disabled={uploading}>
                {uploading ? "⏳ Subiendo..." : "📎 Adjuntar PDF / imagen"}
              </button>
            )}
          </div>
        </div>
...
// ─── BLOQUE SUBVENCIMIENTOS (dentro de tabla/card) ───────────────────────────
function SubvencimientosBloque({ cert, subvencimientos, onAdd, onEdit, onDelete, notify }) {
  const [expanded, setExpanded] = useState(false);
  const svDeCert = subvencimientos.filter(s => s.certificado_id === cert.id);
  const hayProximos = svDeCert.some(s => {
    const d = diasHasta(s.fecha_hasta);
  ...
  return (
    <div className="sv-block" style={{ marginTop: 6 }}>
      <div className="sv-header" onClick={() => setExpanded(e => !e)}>
        ...
        {svDeCert.length > 0 && <span style={{ background: "#BBF7D0", color: "#166534", fontSize: 9, padding: "1px 7px", borderRadius: 10, fontFamily: "var(--mono)", fontWeight: 700 }}>{svDeCert.length}</span>}
        {hayProximos && <span className="badge b-amber" style={{ fontSize: 8, marginLeft: "auto" }}>⚠ Próxima</span>}
      </div>
      {expanded && (
        <div className="sv-list">
          {svDeCert.length === 0 && (
            <div style={{ padding: "8px 12px", fontSize: 11, color: "#6381A7" }}>Sin verificaciones cargadas.</div>
          )}
          {svDeCert.map((s, i) => {
              ...
              <div key={s.id} className="sv-item">
                ...
                <span className="sv-dates">
                  {s.fecha_desde && s.fecha_hasta
                    ...
                    : s.fecha_hasta ? `Hasta: ${fmtDate(s.fecha_hasta)}`
                    : s.fecha_desde ? `Desde: ${fmtDate(s.fecha_desde)}` : "Sin fechas"}
                </span>
                {s.fecha_hasta && <span className={`dias-chip ${alertCls}`} style={{ fontSize: 9 }}>{diasD !== null ? (diasD < 0 ? `Venc. ${Math.abs(diasD)}d` : diasD === 0 ? "Hoy" : `${diasD}d`) : "Sin fecha"}</span>}
                {s.documento_url && (
                  <a href={s.documento_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="sv-doc" title={s.documento_nombre}>📎</a>
                )}
                <button onClick={e => { e.stopPropagation(); onEdit(s); }} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 11, padding: "0 2px" }} title="Editar">✏</button>
                <button onClick={e => { e.stopPropagation(); onDelete(s.id); }} style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", fontSize: 11, padding: "0 2px" }} title="Eliminar">🗑</button>
              </div>
            );
          })}
...
// ─── MODAL: EDITAR/CREAR CERTIFICADO ─────────────────────────────────────────
function ModalCert({ cert, onClose, onSave, notify }) {
  const esNuevo = !cert.id;
  const fileRef = useRef();
  const [form, setForm] = useState({
    buque: cert.buque || "Atlantic Dama",
    tipo: cert.tipo || "estatutario",
    seccion: cert.seccion || "GENERAL",
    descripcion: cert.descripcion || "",
    nro_certificado: cert.nro_certificado || "",
    emitido_por: cert.emitido_por || "",
    nro_serie: cert.nro_serie || "",
    proveedor: cert.proveedor || "",
    fecha_emision: cert.fecha_emision || "",
    fecha_vencimiento: cert.fecha_vencimiento || "",
    fecha_ultimo_servicio: cert.fecha_ultimo_servicio || "",
    fecha_proximo_servicio: cert.fecha_proximo_servicio || "",
    observaciones: cert.observaciones || "",
    documento_url: cert.documento_url || "",
    documento_nombre: cert.documento_nombre || "",
  });
  const [saving, setSaving] = useState(false);
  const [uploading, setUploading] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  const handleUpload = async (file) => {
    if (!file) return;
    const tempId = cert.id || `temp_${Date.now()}`;
    setUploading(true);
    try {
      const { url, nombre } = await api.subirDocumento(file, tempId);
      set("documento_url", url); set("documento_nombre", nombre);
      notify("Documento adjuntado", "success");
    } catch (e) { notify("Error al subir: " + e.message, "error"); }
    finally { setUploading(false); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        buque: form.buque, tipo: form.tipo, seccion: form.seccion,
        descripcion: form.descripcion,
        ...
        fecha_proximo_servicio: form.fecha_proximo_servicio || null,
        observaciones: form.observaciones || null,
        documento_url: form.documento_url || null,
        documento_nombre: form.documento_nombre || null,
        dias_vencimiento: diasHasta(fechaRef),
        activo: true,
      };
      let saved;
      if (cert.id) saved = await api.updateCertificado(cert.id, payload);
      else saved = await api.insertCertificado(payload);
      notify(esNuevo ? "Certificado creado" : "Actualizado", "success");
      onSave(saved);
    } catch (e) { notify("Error: " + e.message, "error"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="mhdr">
          <div className="mtitle">{esNuevo ? "Nuevo certificado" : "Editar certificado"}</div>
          <button className="mclose" onClick={onClose}>✕</button>
        </div>
        <div className="mbody">
          <div className="form-grid">
            <FG label="Buque *">
              <select value={form.buque} onChange={e => set("buque", e.target.value)}>
                {BUQUES.map(b => <option key={b}>{b}</option>)}
              </select>
            </FG>
            <FG label="Tipo *">
              <select value={form.tipo} onChange={e => { set("tipo", e.target.value); set("seccion", e.target.value === "estatutario" ? "GENERAL" : "FFA"); }}>
                <option value="estatutario">Estatutario</option>
                <option value="no_estatutario">No estatutario (Equipo)</option>
              </select>
            </FG>
            <FG label="Sección *">
            ...
            <FG label={esEstat ? "Emitido por" : "Proveedor"}>
              <input value={esEstat ? form.emitido_por : form.proveedor}
                onChange={e => set(esEstat ? "emitido_por" : "proveedor", e.target.value)} />
            </FG>
          </div>
          <FG label="Descripción *" full>
            <input value={form.descripcion} onChange={e => set("descripcion", e.target.value)} placeholder="Nombre del certificado..." />
          </FG>
          <div className="form-grid mt12">
          ...
          {esEstat ? (
            <div className="form-grid">
              <FG label="Fecha emisión"><input type="date" value={form.fecha_emision} onChange={e => set("fecha_emision", e.target.value)} /></FG>
              <FG label="Fecha vencimiento"><input type="date" value={form.fecha_vencimiento} onChange={e => set("fecha_vencimiento", e.target.value)} /></FG>
            </div>
          ) : (
            <div className="form-grid">
              <FG label="Último servicio"><input type="date" value={form.fecha_ultimo_servicio} onChange={e => set("fecha_ultimo_servicio", e.target.value)} /></FG>
              <FG label="Próximo servicio"><input type="date" value={form.fecha_proximo_servicio} onChange={e => set("fecha_proximo_servicio", e.target.value)} /></FG>
            </div>
          )}
          <FG label="Observaciones" full>
            <textarea value={form.observaciones} onChange={e => set("observaciones", e.target.value)} placeholder="Notas..." />
          </FG>
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: .5, textTransform: "uppercase", color: "var(--navy)", marginBottom: 8 }}>Documento adjunto</div>
            <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: "none" }} onChange={e => handleUpload(e.target.files[0])} />
...
function ModalVerCert({ cert, subvencimientos, onClose, onEdit, onDelete, notify, onSubvencimientosChange }) {
  ...
  const [modalSv, setModalSv] = useState(null); // null | "new" | sv_obj
  ...
  const handleDeleteSv = async (svId) => {
    ...
    try {
      await api.deleteSubvencimiento(svId);
      notify("Verificación eliminada", "success");
      onSubvencimientosChange();
    } catch (e) { notify("Error: " + e.message, "error"); }
  };
  ...
  return (
    <>
        ...
        <div className="modal modal-wide">
          ...
          <div className="mbody">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {esEstat ? <>
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Emitido por</div>{cert.emitido_por || "—"}</div>
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>N° Certificado</div>{cert.nro_certificado || "—"}</div>
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Fecha emisión</div>{fmtDate(cert.fecha_emision)}</div>
                <div className="info-box" style={{ background: dias !== null && dias < 0 ? "#FEF2F2" : dias !== null && dias <= 30 ? "#FFF7ED" : undefined }}>
                  <div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Fecha vencimiento</div>
                ...
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Proveedor</div>{cert.proveedor || "—"}</div>
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>N° Certificado</div>{cert.nro_certificado || "—"}</div>
                <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Último servicio</div>{fmtDate(cert.fecha_ultimo_servicio)}</div>
                <div className="info-box" style={{ background: dias !== null && dias < 0 ? "#FEF2F2" : dias !== null && dias <= 30 ? "#FFF7ED" : undefined }}>
                  <div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>Próximo servicio</div>
              ...
              {cert.nro_serie && <div className="info-box"><div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 3, letterSpacing: 1, textTransform: "uppercase" }}>N° Serie</div>{cert.nro_serie}</div>}
            </div>
            {cert.observaciones && <div className="info-box mt8" style={{ fontSize: 12 }}><strong>Obs:</strong> {cert.observaciones}</div>}
            {cert.documento_url && (
              <div className="doc-adjunto mt8">
                <span>📎</span>
                <a href={cert.documento_url} target="_blank" rel="noreferrer" style={{ color: "var(--blue)", flex: 1 }}>{cert.documento_nombre || "Ver documento adjunto"}</a>
              </div>
            )}

            {/* SUBVENCIMIENTOS en modal ver */}
            {esEstat && (
              <div className="mt12">
                <SubvencimientosBloque
                  cert={cert}
                  subvencimientos={subvencimientos}
                  onAdd={() => setModalSv("new")}
                  onEdit={sv => setModalSv(sv)}
                  onDelete={handleDeleteSv}
                  notify={notify}
                />
              </div>
            )}
            ...
            {confirmDelete && (
              <div className="info-box danger mt12" style={{ fontSize: 12 }}>
      ...
      {/* Modal subvencimiento anidado */}
      {modalSv && (
        <ModalSubvencimiento
          certId={cert.id}
          sv={modalSv === "new" ? null : modalSv}
          onClose={() => setModalSv(null)}
          onSave={() => { setModalSv(null); onSubvencimientosChange(); }}
          notify={notify}
        />
      )}
    </>
...
function PrintModal({ buque, tipo, certs, subvencimientos, onClose }) {
  ...
  const certNums = {};
  secciones.forEach(s => {
    (grupos[s] || []).forEach(c => { globalNum++; certNums[c.id] = globalNum; });
  });
    ...
    <div className="print-overlay no-print" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="print-modal">
        <div className="print-modal-header no-print">
          <div>
            <div style={{ fontWeight: 700, fontSize: 14 }}>Vista previa de impresión</div>
            <div style={{ fontSize: 11, opacity: .7, marginTop: 2 }}>{buque} — {tipoLabel}</div>
          </div>
        ...
        <div className="print-body" id="print-content">
          <div className="print-vessel-title">{buque}</div>
          <div className="print-vessel-sub">
            <span>{tipoLabel} — Estado al {fechaHoyStr}</span>
            <span style={{ fontFamily: "var(--mono)", fontSize: 10 }}>Parana Logística S.A.</span>
          </div>
          ...
          {secciones.filter(s => (grupos[s] || []).length > 0).map(seccion => {
            ...
            return (
              <div key={seccion}>
                <div className="print-sec-header">
                ...
                <table className="print-table">
                  ...
                  <tbody>
                    {items.map(c => {
                      ...
                      const dias = diasHasta(fechaRef);
                      const alertColor = dias !== null && dias < 0 ? "var(--danger)" : dias !== null && dias <= 30 ? "var(--orange)" : dias !== null && dias <= 90 ? "var(--warn)" : "inherit";
                      const svDeCert = subvencimientos.filter(s => s.certificado_id === c.id);
                      return (
                        <>
                          <tr key={c.id} style={{ background: dias !== null && dias < 0 ? "#FFF5F5" : dias !== null && dias <= 30 ? "#FFFAF5" : "inherit" }}>
                            ...
                            <td style={{ fontFamily: "var(--mono)", fontSize: 9, fontWeight: 700, color: alertColor }}>{tipo === "estatutario" ? fmtDate(c.fecha_vencimiento) : fmtDate(c.fecha_proximo_servicio)}</td>
                            <td><PrintChip fechaStr={fechaRef} /></td>
                          </tr>
                          {/* SUBVENCIMIENTOS EN PRINT */}
                          {svDeCert.map((s, si) => (
                            <tr key={s.id} className="print-sv-row">
                              <td style={{ textAlign: "center", color: "#888", fontSize: 8 }}>{certNums[c.id]}.{si + 1}</td>
                              <td colSpan={4}>
                                <span className="print-sv-arrow">↳</span>
                                <span style={{ fontWeight: 500 }}>{s.descripcion}</span>
                                {s.observaciones && <span style={{ color: "#888", marginLeft: 6 }}>· {s.observaciones}</span>}
                              </td>
                              <td style={{ fontFamily: "var(--mono)", fontSize: 9 }}>
                                {s.fecha_desde && s.fecha_hasta
                                  ? `${fmtDate(s.fecha_desde)} → ${fmtDate(s.fecha_hasta)}`
                                  : s.fecha_hasta ? fmtDate(s.fecha_hasta) : "—"}
                              </td>
          ...
          <div className="print-footer">
            ...
            <span>{fechaHoyStr} · {certsBuque.length} certificados</span>
          </div>
        </div>
      </div>
    </div>
...
// ─── PAGE: TABLA ──────────────────────────────────────────────────────────────
function PageTabla({ certs, buque, tipo, subvencimientos, onSelect, onNuevo, onSubvencimientosChange, notify }) {
  const [filtroSeccion, setFiltroSeccion] = useState("");
  const [busqueda, setBusqueda] = useState("");
  const [showPrint, setShowPrint] = useState(false);
  const [modalSv, setModalSv] = useState(null); // { certId, sv | null }
        ...
        : secciones.filter(s => grupos[s]?.length > 0).map(seccion => (
          <div key={seccion} className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 14 }}>
            <div style={{ padding: "8px 14px", background: "var(--navy)", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {grupos[seccion].filter(c => (diasHasta(getFechaRef(c)) || 0) < 0).length > 0 && (
                <span className="badge b-red" style={{ marginLeft: "auto", fontSize: 9 }}>{grupos[seccion].filter(c => (diasHasta(getFechaRef(c)) || 0) < 0).length} vencidos</span>
              )}
              {grupos[seccion].filter(c => { const d = diasHasta(getFechaRef(c)); return d !== null && d >= 0 && d <= 30; }).length > 0 && (
                <span className="badge b-orange" style={{ fontSize: 9 }}>{grupos[seccion].filter(c => { const d = diasHasta(getFechaRef(c)); return d !== null && d >= 0 && d <= 30; }).length} críticos</span>
              )}
            </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Descripción</th>
                    {tipo === "estatutario" ? <>
                      <th>N° Cert.</th><th>Emitido por</th>
                      <th>Fecha emisión</th><th>Fecha vencimiento</th>
                    </> : <>
                      <th>N° Cert.</th><th>Proveedor</th>
                      <th>Último servicio</th><th>Próximo servicio</th>
                    </>}
                    <th>Estado</th>
                    <th>Doc.</th>
                  </tr>
                ...
                <tbody>
                  {grupos[seccion].map(c => {
                    ...
                    return (
                      <>
                        <tr key={c.id} className="click" style={{ background: rowBg }} onClick={() => onSelect(c)}>
                          <td className="num-col">{certNums[c.id]}</td>
                          <td style={{ fontWeight: 500, fontSize: 12, maxWidth: 260 }}>
                            {c.descripcion}
                            {tipo === "estatutario" && svDeCert.length > 0 && (
                              <span className="badge b-teal" style={{ marginLeft: 6, fontSize: 8, verticalAlign: "middle" }}>
                                {svDeCert.length} verif.
                              </span>
                            )}
                          </td>
                          ...
                          <td className="text-mono" style={{ fontSize: 11, color: "var(--muted)" }}>
                            {tipo === "estatutario" ? fmtDate(c.fecha_emision) : fmtDate(c.fecha_ultimo_servicio)}
                          </td>
                          ...
                          <td style={{ textAlign: "center" }}>
                            {c.documento_url
                              ? <a href={c.documento_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 14, color: "var(--blue)" }}>📎</a>
                              : <span style={{ color: "var(--muted2)", fontSize: 11 }}>—</span>
                            }
                          </td>
                        </tr>
                        {/* Fila subvencimientos inline */}
                        {tipo === "estatutario" && (
                          <tr key={`sv-${c.id}`}>
                            <td colSpan={8} style={{ padding: 0, paddingLeft: 40, paddingRight: 12, background: rowBg }}>
                              <SubvencimientosBloque
                                cert={c}
                                subvencimientos={subvencimientos}
                                onAdd={() => setModalSv({ certId: c.id, sv: null })}
                                onEdit={sv => setModalSv({ certId: c.id, sv })}
                                onDelete={handleDeleteSv}
                                notify={notify}
                              />
                            </td>
                ...
                return (
                  <div key={c.id} className="mobile-card" onClick={() => onSelect(c)}>
                    ...
                    <div className="mobile-card-meta">
                      {tipo === "estatutario" ? `Vence: ${fmtDate(c.fecha_vencimiento)}` : `Próx. servicio: ${fmtDate(c.fecha_proximo_servicio)}`}
                      {c.documento_url && <span style={{ marginLeft: 8, color: "var(--blue)" }}>📎</span>}
                    </div>
                    {svDeCert.length > 0 && (
                      <div className="mobile-card-sv">
                        {svDeCert.slice(0, 2).map(s => (
                          <div key={s.id}>↳ {s.descripcion}{s.fecha_hasta ? ` · ${fmtDate(s.fecha_hasta)}` : ""}</div>
                        ))}
...
function PageAlertas({ certs, subvencimientos, onSelect }) {
  ...
  // ── ITEMS EN RANGO: certificados + subvencimientos ────────────────────────
  // Cada item tiene: tipo_item ("cert" | "subvenc"), fechaRef, cert (obj), sv (obj | null)
  const itemsEnRango = [];
  ...
  certs.filter(c => !filtroBuque || c.buque === filtroBuque).forEach(c => {
    ...
    if (f && f >= desde && f <= hasta) {
      itemsEnRango.push({ tipo_item: "cert", fechaRef: f, cert: c, sv: null });
    }
    // Subvencimientos del certificado
    subvencimientos
      .filter(s => s.certificado_id === c.id)
      .forEach(s => {
        const fSv = s.fecha_hasta; // la fecha de cierre de ventana es la relevante
        if (fSv && fSv >= desde && fSv <= hasta) {
          itemsEnRango.push({ tipo_item: "subvenc", fechaRef: fSv, cert: c, sv: s });
        }
      });
  });
  ...
  return (
    <div>
      <div className="stats">
        <div className="stat"><div className="stat-label">Total certificados</div><div className="stat-value sv">{certs.length}</div></div>
      ...
      <div className="card mb12">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <FG label="Barco">
            <select value={filtroBuque} onChange={e => setFiltroBuque(e.target.value)}>
        ...
        <div className="flex-gap mt8">
          <div className="info-box accent" style={{ fontSize: 11, flex: 1 }}>
            ...
            {itemsEnRango.filter(i => diasHasta(i.fechaRef) < 0).length > 0 &&
              <span style={{ color: "var(--danger)", marginLeft: 8 }}>· {itemsEnRango.filter(i => diasHasta(i.fechaRef) < 0).length} ya vencidos</span>}
            {itemsEnRango.some(i => i.tipo_item === "subvenc") &&
              <span className="badge b-teal" style={{ marginLeft: 8, fontSize: 9 }}>
                {itemsEnRango.filter(i => i.tipo_item === "subvenc").length} verif. incluidas
              </span>}
          </div>
        ...
        <div className="info-box success mt8" style={{ fontSize: 10 }}>
          ℹ️ El rango incluye los vencimientos de <strong>certificados principales</strong> y también los de sus <strong>verificaciones periódicas / subvencimientos</strong>.
        </div>
      </div>
        ...
        : (
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: "var(--muted)", textTransform: "uppercase", marginBottom: 10 }}>
              {itemsEnRango.length} vencimientos en el período
            </div>
            {itemsEnRango.map((item, idx) => {
              const { tipo_item, fechaRef, cert: c, sv } = item;
              const alertCls = getAlertClass(fechaRef);
              const esSubvenc = tipo_item === "subvenc";

              return (
                <div
                  key={`${esSubvenc ? "sv" : "c"}-${esSubvenc ? sv.id : c.id}-${idx}`}
                  className={`alert-row ${alertCls}`}
                  style={esSubvenc ? { marginLeft: 24, borderLeftStyle: "dashed", opacity: .95 } : {}}
                  onClick={() => onSelect(c)}
                >
                  ...
                  {esSubvenc ? (
                    <>
                      <div style={{ fontWeight: 600, fontSize: 13, color: "var(--navy)" }}>
                        <span style={{ fontSize: 10, color: "var(--muted)", fontWeight: 400 }}>↳ {c.descripcion} · </span>
                        {sv.descripcion}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>
                        Ventana: {sv.fecha_desde ? fmtDate(sv.fecha_desde) : "—"} → {fmtDate(sv.fecha_hasta)}
                        {sv.observaciones && <span style={{ marginLeft: 8 }}>· {sv.observaciones}</span>}
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>
                        {c.tipo === "estatutario"
                          ? `Vence: ${fmtDate(c.fecha_vencimiento)}${c.emitido_por ? ` · ${c.emitido_por}` : ""}`
                          : `Próx. servicio: ${fmtDate(c.fecha_proximo_servicio)}${c.proveedor ? ` · ${c.proveedor}` : ""}`
                        }
                      </div>
                    </>
                  )}
...
export default function App() {
  ...
  const loadCerts = useCallback(async () => {
    setLoading(true);
    try {
      const [c, sv] = await Promise.all([api.getCertificados(), api.getAllSubvencimientos()]);
      setCerts(c);
      setSubvencimientos(sv);
    } catch (e) { notify("Error: " + e.message, "error"); }
    finally { setLoading(false); }
  }, []);

  const reloadSubvencimientos = useCallback(async () => {
    try { setSubvencimientos(await api.getAllSubvencimientos()); }
    catch (e) { notify("Error al recargar verificaciones: " + e.message, "error"); }
  }, []);
  ...
  if (checkingAuth) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#213363" }}>
      <style>{CSS}</style>
      <div className="loading" style={{ color: "rgba(255,255,255,.5)" }}><span className="spin">◌</span></div>
    </div>
  );
  ...
  const pageTitles = {
    ...
    "ad-estat": "ATLANTIC DAMA — ESTATUTARIOS",
    "ad-equipo": "ATLANTIC DAMA — EQUIPOS",
    "gdm-estat": "GOLONDRINA DE MAR — ESTATUTARIOS",
    "gdm-equipo": "GOLONDRINA DE MAR — EQUIPOS",
  };

  const pageConfig = {
    "ad-estat":   { buque: "Atlantic Dama",     tipo: "estatutario" },
    "ad-equipo":  { buque: "Atlantic Dama",     tipo: "no_estatutario" },
    "gdm-estat":  { buque: "Golondrina de Mar", tipo: "estatutario" },
    "gdm-equipo": { buque: "Golondrina de Mar", tipo: "no_estatutario" },
  };
  ...
  const Sidebar = () => (
    <nav className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      ...
      <div style={{ padding: "10px 16px", borderTop: "1px solid rgba(255,255,255,.1)" }}>
        <div style={{ fontSize: 9, color: "rgba(255,255,255,.35)", fontFamily: "var(--mono)", marginBottom: 6, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {session.user.email}
        </div>
  ...
  return (
    <>
        ...
        <div className="main">
          <div className="topbar">
            ...
            <div className="flex-gap">
              ...
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: "#DBEAFE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "var(--blue)", fontWeight: 700 }}>
          ...
          <div className="content">
            {loading ? <div className="loading"><span className="spin">◌</span> Cargando...</div> : <>
              ...
              {pageConfig[page] && (
                <PageTabla
                  certs={certs}
                  buque={pageConfig[page].buque}
                  tipo={pageConfig[page].tipo}
                  subvencimientos={subvencimientos}
                  onSelect={c => setSelected(c)}
                  ...
                  onSubvencimientosChange={reloadSubvencimientos}
                  notify={notify}
                />
              )}
            </>}
      ...
      {(editando || creando) && (
        <ModalCert
          cert={editando || creando}
          onClose={() => { setEditando(null); setCreando(null); }}
          onSave={saved => {
            if (editando) setCerts(prev => prev.map(c => c.id === saved.id ? saved : c));
            else setCerts(prev => [...prev, saved]);
            setEditando(null); setCreando(null);
          }}
          notify={notify}
        />
      )}
